...
function assertSignInUrlFormatAndOrigin(_signInUrl: string, frontendApi: string) {
  let signInUrl: URL;
  try {
  ...
  if (signInUrl.hostname === frontendApi.replace('clerk.', '')) {
...
export async function authenticateRequest(options: AuthenticateRequestOptions): Promise<RequestState> {
  ...
  if (options.isSatellite) {
    ...
    assertSignInUrlFormatAndOrigin(options.signInUrl, options.frontendApi as string);
